...
if __name__ == "__main__":
    ...
    move_skill.execute(env, [state], np.array([[state[0]+0.15, 0.1]]), 1, 50, set_state=False)
    move_skill.execute(env, [state], np.array([[state[0]+0.25, 0.2]]), 1, 50, set_state=False)
    move_skill.execute(env, [state], np.array([[state[0]+0.25, 0.4]]), 1, 50, set_state=False)
    state_after = env.get_sem_state()
    pour_skill.execute(env, [state], parameters, 1, 50, set_state=False)
    pour_skill.execute(env, [state], parameters, 1, 50, set_state=False)
    pour_skill.execute(env, [state], np.array([[0]]), 1, 50, set_state=False)
